...
- documents*id: The unique identifier (uuid4) used for the set of documents being compared in this HIT. This corresponds to a object in either \_DocumentPairSetsForHITS.json* or _DocumentRankingSetsForHITS.json_.
...
Here are the views we deemed important for analyzing the data as we received it. This is also for future reference, for other crowdsourcing projects we'll do.

To show only active (non-disposed) HITs:
    ...
    [
        ...
        },
        {
            ...
            "timestamp": 1,
            "Expire": "$hit.Expiration",
            "AssignmentsAvailable": "$hit.NumberOfAssignmentsAvailable",
            "AssignmentsPending": "$hit.NumberOfAssignmentsPending",
            "AssignmentsCompleted": "$hit.NumberOfAssignmentsCompleted"
            }
        }
    ]


To unwind and show each individual answer in a document:
    ...
    [
        ...
        {
            "$addFields": {
            "output": "$answers.values.outputs",
            "worker_id": "$answers.worker_id",
            "feedback": "$answers.values.feedback"
            }
        },
        {
            "$unwind": {
...
For seing each batch of HITs, along with count of HITs and average payment:
...
To see a count of how many assignments we have per worker:
